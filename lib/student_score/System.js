...
let status = STATUS.MAIN_DEFAULT;
...
class System {
    ...
    resolveStudentInfo(input) {
        ...
        if(student_info.length < 8) {
            ...
            let math = this.parseStudetenSubject(student_info[4]);
            let chinese = this.parseStudetenSubject(student_info[5]);
            let english = this.parseStudetenSubject(student_info[6]);
            let program = this.parseStudetenSubject(student_info[7]);
