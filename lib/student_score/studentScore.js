let Student = require('./Student');
let Class = require('./Class');

let allCls = [];
const readline = require('readline');
const rl = readline.createInterface({
    input: process.stdin,
    output: process.stdout
});

entry();
function entry() {
    printSysInfo();
    rl.on('line', (input) => {
        if (input === '1') {
            let output = '请输入学生信息（格式：姓名，学号，民族，班级，学科：成绩，...），按回车提交：';
            handAddStudent(output);
        } else if (input === '2') {
            let output = '请输入要打印的学生的学号：（格式：学号，学号...）';
            printStudentInfo(output);
        } else {
            process.exit(1);
        }
    })
}

function handAddStudent(output) {
    rl.question(output, (answer) => {
        let stu = resolveStudentInfo(answer);
        if(Object.prototype.toString.call(stu) == '[object Object]') {
            allCls = addStudentScore(stu, allCls);
            console.log('添加学生成功');
            console.log(allCls);
            printSysInfo();
            return;
        } else {
            handAddStudent('请按正确的格式输入（格式：姓名，学号，学科：成绩，...）');
        }
    })
}

function printStudentInfo(output) {
    rl.question(output, answer => {
        let stuNos = resolveStudId(answer);
        if(Object.prototype.toString.call(stuNos) === '[object Array]') {
            let stuStr = getStudentScore(stuNos, allCls);
            console.log(stuStr);
            printSysInfo();
        } else {
            printStudentInfo('请按正确的格式输入（格式：学号，学号，...）');
        }
    })
}

function printSysInfo() {
    console.log('1.添加学生\n2.生成成绩单\n3.退出\n请输入你的选择(1-3)');
}
function addStudentScore(student, allCls) {
    if(student !== null) {
        let stuScore =  student.calStuScore(student);
        let findcl = allCls.find(cl => cl.findClass(stuScore));
        if(findcl != null) {
            let index = findcl.findStudent(stuScore);
            if(index !== -1) {
                findcl.stuScore.splice(index, 1);
            }
            let avarage = findcl.calAverage(stuScore);
            let middle = findcl.calMiddle(stuScore);
            findcl.updateClass(stuScore, avarage, middle);
        } else {
            addClass(allCls, stuScore);
        }
    }
    return allCls;
}

function getStudentScore(test, allcls) {
    let newcls = [];
    for(let stuno of test) {
        //let findcl = allcls.find(cl => cl.findStuScore(stuno));
        let findcl = allcls.find(cl => cl.stuScore.filter(stu => stu.student.stuno == stuno));
        if(findcl != null) {
            findcl.updateNewClass(newcls);
        }
    }
    let str = printStuScore(newcls);
    return str;
}

function resolveStudentInfo(input) {
    let student_info = input.split(',');
    if(student_info.length < 8) {
        return '请按正确的格式输入（格式：姓名，学号，学科：成绩，...）';
    } else {
        let math = student_info[4].split(':')[1];
        let chinese = student_info[5].split(':')[1];
        let english = student_info[6].split(':')[1];
        let program = student_info[7].split(':')[1];
        let student = new Student({
            name: student_info[0],
            stuno: parseInt(student_info[1]),
            nation: student_info[2],
            classno: parseInt(student_info[3]),
            math: parseFloat(math),
            chinese: parseFloat(chinese),
            english: parseFloat(english),
            program: parseFloat(program)
        });
        return student;
    }
    return null;
}

function resolveStudId(stuIds) {
    let ids = stuIds.split(',');
    let res = [];
    for(let id of ids) {
        if(isNaN(id)) {
            return null;
        }
        res.push(parseInt(id));
    }
    return res;
}

function printStuScore(newcl) {
    let stuStr = '';
    if(newcl.length === 0) {
        stuStr = '该学号没有对应的班级信息';
    }
    for(let cl of newcl) {
        stuStr += '成绩单\n姓名|数学|语文|英语|编程|平均分|总分\n';
        stuStr += '========================\n';
        for(let stu of cl.stuScore) {
            stuStr += stu.student.name + '|';
            stuStr += stu.student.math + '|';
            stuStr += stu.student.chinese + '|';
            stuStr += stu.student.english + '|';
            stuStr += stu.student.program + '|';
            stuStr += stu.average + '|';
            stuStr += stu.sum + '\n';
        }
        stuStr += '========================\n';
        stuStr += '全班总分平均分:'+cl.average + '\n';
        stuStr += '全班总分中位数:'+cl.middle + '\n';
    }
    return stuStr;
}

function addClass(allClass, stuScore) {
    let newcl = new Class(stuScore.sum, stuScore.sum, stuScore.student.classno);
    newcl.addStuScore(stuScore);
    allClass.push(newcl);
    return allClass;
}

module.exports = {resolveStudentInfo, addClass, resolveStudId, printStuScore};